function Spin(id, options) {
    ...
    var innityAppsPreviousIndex = -1, innityAppsIsSpinEnded = false;
    ...
    function calcSectorSnaped(snapAngle, direction) {
        ...
        trackerHandler(calcDesignerIndex_());
    };
    function clickOnSector() {
        if (typeof(extraOptions.sectorClickedCallback) === 'function') {
            extraOptions.sectorClickedCallback(calcDesignerIndex_());
        }
    }
    ...
    function cancelAutoAnimation() {
        ...
        innityAppsIsSpinEnded = true;
    }

    function trackerHandler(sector) {
        if (innityAppsIsSpinEnded === false) {
        return;
        }
    
        if (sector === innityAppsPreviousIndex) {
        return;
        }

        var spinTracker = (typeof innityAppsTriggerTrack === 'function') ? innityAppsTriggerTrack : triggerTrack;
        spinTracker(`sector_${sector}`);
        innityAppsPreviousIndex = sector;
    } 
